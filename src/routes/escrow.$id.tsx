        ...
        {TIMELINE.map((s, i) => {
          ...
          return (
            <div key={s.key} className="flex flex-1 items-center gap-2">
              <div
                className={`flex items-center gap-2 ${current ? "text-primary" : done ? "text-emerald-400" : "text-muted-foreground"}`}
              >
                <div
                  className={`grid h-7 w-7 place-items-center rounded-full border ${current ? "border-primary bg-primary/15 animate-pulse" : done ? "border-emerald-500/50 bg-emerald-500/10" : "border-border/60 bg-secondary/30"}`}
                >
...
function EscrowGroupPage() {
  ...
  useEffect(() => {
    const ch = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "escrow_group_messages", filter: `group_id=eq.${id}` },
        () => refetch(),
      )
      .on("postgres_changes", { event: "*", schema: "public", table: "escrow_groups", filter: `id=eq.${id}` }, () =>
        refetch(),
      )
      .subscribe();
    return () => {
    ...
    return (
      <div className="mx-auto max-w-xl surface p-8 text-center space-y-4">
        ...
        <h1 className="text-xl font-semibold">
          {notFound
            ? "Escrow group not found"
            : forbidden
              ? "You don't have access to this escrow"
              : "Could not open this escrow group"}
        </h1>
        ...
        <div className="flex flex-wrap justify-center gap-2">
          <Link to="/escrow">
            <Button variant="outline">Back to my escrow</Button>
          </Link>
          <Link to="/escrow/new">
  ...
  const myMember = data.members.find((m) => m.user_id === user?.id) as
    | { accepted_at: string | null; declined_at: string | null; role: string }
    | undefined;
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h1 className="text-3xl font-semibold">
              Trade room · {g.amount} {g.asset}
            </h1>
            <p className="mt-2 text-sm text-muted-foreground">
              This is your escrow group for the trade. Chat, track deposits, accept invites, and sync with Telegram in
              one place.
            </p>
          </div>
        ...
        {showInvitePanel && (
          <div className="mt-5 rounded-2xl border border-primary/30 bg-primary/5 p-4">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Share this link with the seller. They'll be added to the group as soon as they sign in and accept.
            </p>
        ...
        <div className="space-y-4">
          ...
          <div className="surface p-5">
            ...
            <div className="mt-4 grid gap-2 text-sm">
              <div className="flex justify-between">
                <span className="text-muted-foreground">Buyer (group creator)</span>
                <span>{data.members.find((m) => m.role === "buyer")?.profile?.display_name ?? "—"}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Seller</span>
                <span>
                  {data.members.find((m) => m.role === "seller")?.profile?.display_name ??
                    g.invited_username ??
                    g.invited_telegram ??
                    "(awaiting acceptance)"}
                </span>
              ...
              <div className="flex justify-between">
                <span className="text-muted-foreground">Escrow address ({g.escrow_address_chain ?? g.asset})</span>
                <span className="font-mono text-xs">
                  {g.escrow_address ?? <em className="text-muted-foreground">seller has no payout address yet</em>}
                </span>
              </div>
              {g.deposit_tx_hash && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Deposit tx</span>
                  <span className="font-mono text-xs">{g.deposit_tx_hash}</span>
                </div>
              )}
            </div>
          </div>
          ...
          {iAmPendingInvitee && (
            <div className="surface p-5 border-emerald-200/60">
              <div className="flex flex-col gap-3">
                ...
                <p className="text-sm text-muted-foreground">
                  You were invited into this escrow group as a {myMember?.role}. Accept the invite to join the group and
                  continue the transaction.
                </p>
                <div className="flex flex-wrap gap-2">
                  <Button onClick={() => act(() => accept({ data: { group_id: g.id } }), "Invite accepted")}>
                    Accept invite
                  </Button>
                  <Button
                    variant="outline"
                    onClick={() => act(() => decline({ data: { group_id: g.id } }), "Invite declined")}
                  >
                    Decline invite
                  </Button>
                </div>
                  ...
                  <div className="mt-4 space-y-3 text-sm">
                    <div className="rounded-2xl bg-secondary/50 p-4">
                      ...
                      <div className="mt-2 font-mono text-xs break-all">
                        {sellerWallet ? (
                          blurAddress(sellerWallet)
                        ) : (
                          <span className="text-muted-foreground">Seller has not added a {g.asset} address.</span>
                        )}
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="font-mono">
                        {g.amount} {g.asset}
                        {g.fiat_amount ? ` · ≈ ${g.fiat_amount} ${g.fiat_currency}` : ""}
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="mt-4 space-y-3">
                    <div className="rounded-2xl bg-background p-4 text-center">
                      ...
                      <div className="text-sm text-muted-foreground font-mono">
                        ≈ {heldFiat.toLocaleString()} {g.fiat_currency}
                      </div>
                    ...
                    <div className="rounded-2xl bg-background p-4">
                      <div className="text-[10px] uppercase text-muted-foreground">
                        Escrow address ({g.escrow_address_chain ?? g.asset})
                      </div>
                      <div className="mt-2 font-mono text-xs break-all">
                        {g.escrow_address ?? "Waiting for seller payout address."}
                      </div>
                    </div>
                    <div className="rounded-2xl bg-background p-4">
                      <div className="flex items-center justify-between">
                      ...
                      {g.deposit_tx_hash ? (
                        ...
                        <div className="mt-2 flex flex-col gap-2">
                          <Input
                            ...
                            placeholder={`Paste ${g.asset} tx hash`}
                            className="font-mono"
                          />
                          <Button
                            onClick={() => act(() => submitHash({ data: { group_id: g.id, hash } }), "Hash submitted")}
                            disabled={!hash.trim()}
                          >
                            Submit payment
                          </Button>
                ...
                <div className="rounded-3xl border border-border/60 p-5 order-3">
                  ...
                  <div className="mt-4 space-y-3 text-sm">
                    <div className="rounded-2xl bg-secondary/50 p-4">
                      ...
                      <div className="mt-2 font-mono text-xs break-all">
                        {buyerWallet ? (
                          blurAddress(buyerWallet)
                        ) : (
                          <span className="text-muted-foreground">Buyer has not added a {g.asset} address.</span>
                        )}
                      </div>
                    </div>
                    <div>
          ...
          {isSeller && !iAmPendingInvitee && g.status === "funded" && (
            <div className="surface p-5 space-y-3">
              <div className="flex items-center gap-2 font-semibold text-emerald-400">
                <CheckCircle2 className="h-4 w-4" /> Verify deposit & release
              </div>
              <p className="text-xs text-muted-foreground">
                Buyer submitted tx <code className="font-mono">{g.deposit_tx_hash}</code>. Check the{" "}
                {g.escrow_address_chain ?? g.asset} chain to confirm {g.amount} {g.asset} arrived at{" "}
                <code className="font-mono">{g.escrow_address}</code>.
              </p>
              <div className="flex flex-wrap gap-2">
                {!g.deposit_verified_at ? (
                  <Button
                    variant="outline"
                    onClick={() => act(() => verify({ data: { group_id: g.id } }), "Deposit marked verified")}
                  >
                ...
                <Button
                  disabled={!g.deposit_verified_at}
                  onClick={() => act(() => release({ data: { group_id: g.id } }), "Released")}
                >
          ...
          <div className="surface p-5">
            <div className="flex flex-wrap gap-2">
              <Button
                variant="outline"
                onClick={() => act(() => invMod({ data: { group_id: g.id } }), "Moderator invited")}
              >
              ...
              {!["released", "cancelled"].includes(g.status) && (
                <Button variant="ghost" onClick={() => act(() => cancel({ data: { group_id: g.id } }), "Cancelled")}>
                  Cancel group
                </Button>
              )}
              <Link to="/escrow/new">
            ...
            {tgDeepLink && (
              <p className="mt-3 text-[11px] text-muted-foreground">
                Telegram tip: create a group in Telegram, add the bot, then send{" "}
                <code>/start {g.telegram_link_token}</code> to bind it. Once bound, every message in this chat mirrors
        ...
        <div className="surface flex h-[640px] flex-col">
          ...
          <div className="flex-1 space-y-2 overflow-y-auto p-3 text-sm">
            {data.messages.map((m) => (
              <div
                key={m.id}
                className={`rounded-md p-2 ${m.is_system ? "border border-dashed border-border/60 text-xs text-muted-foreground" : m.sender_id === user?.id ? "ml-8 bg-primary/15" : "mr-8 bg-secondary/40"}`}
              >
                {!m.is_system && m.sender?.display_name && (
                  <div className="mb-0.5 text-[10px] font-semibold text-muted-foreground">
                    {m.sender.display_name}
                    {m.from_telegram ? " · via TG" : ""}
                  </div>
                )}
                {m.body}
          ...
          <form
            ...
            onSubmit={async (e) => {
              ...
              if (!msg.trim()) return;
              try {
                await sendMsg({ data: { group_id: g.id, body: msg } });
                setMsg("");
                refetch();
              } catch (e) {
                toast.error((e as Error).message);
